...
sap.ui.define([
	 ...
	 */
	var RadialMicroChart = Control.extend("com.globalintelli.ZAE_SCIN_NEW.controller.microchart.RadialMicroChart", /** @lends sap.suite.ui.microchart.RadialMicroChart.prototype */ {
		/**
	...
	RadialMicroChart.prototype.onBeforeRendering = function () {
		if (!this._getPercentageMode()) {
			if (this.getTotal() === 0) {
				var infomsg = this.getView().getModel("i18n").getResourceBundle().getText("Totalcannotbe0Pleaseaddavalidtotalvalue");
				Log.info(infomsg);
			} else {
	...
	RadialMicroChart.prototype._onResize = function () {
		...
		} else if (iControlHeight < this.convertRemToPixels(RadialMicroChart.THRESHOLD_LOOK_M) || iControlWidth < this.convertRemToPixels(
				RadialMicroChart.THRESHOLD_LOOK_M)) {
			$Control.addClass("sapSuiteRMCLookS");
			$OutsideLabelDiv.addClass("sapSuiteRMCLabelHide");
		} else if (iControlHeight < this.convertRemToPixels(RadialMicroChart.THRESHOLD_LOOK_L) || iControlWidth < this.convertRemToPixels(
				RadialMicroChart.THRESHOLD_LOOK_L)) {
	...
	RadialMicroChart.prototype._getAltHeaderText = function (bIsActive) {
		...
		if (fPercentage <= 0) {
			...
			if (this._isValueColorValid()) {
				...
				sTextValue += Number(this.getPercentage()) / 10 + "   " + this._getStatusText();
			} else {
				// sTextValue += this._oRb.getText("RADIALMICROCHART_PERCENTAGE_TEXT", fPercentage);
				sTextValue += Number(fPercentage) / 10; // Customer code by Jayaramu
			}
		}
		return sTextValue;
	};

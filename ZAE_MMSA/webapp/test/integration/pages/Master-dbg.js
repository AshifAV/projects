		...
		onTheMasterPage : {
			...
			actions : {
				...
				iSortTheListOnName : function () {
					return this.iChooseASorter("sortButton", "Sort By <ServiceDocumentDescription>");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "ZAE_I_ServiceRequest_03",
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("ServiceRequest") === oObject.ServiceRequest;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].ServiceRequest;
									} else {
										sCurrentId = aItemsNotInTheList[0].ServiceRequest;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ZAE_I_ServiceRequest_03", {
										ServiceRequest : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("ServiceRequest"),
						title: oBindingContext.getProperty("ServiceDocumentDescription")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<ZAE_I_ServiceRequest_03> (0)"}),
						success: function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("ServiceDocumentDescription");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all ZAE_I_ServiceRequest_03 to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<ZAE_I_ServiceRequest_03> (" + iExpectedLength + ")"}),
								success : function () {
